...
class StyleShot(torch.nn.Module):
    ...
    @torch.inference_mode()
    def get_image_embeds(self, style_image=None, neg_content_embd=None, less_condition=False, neg_content_name=None, n=2):
        ...
        # print(style_embeds[:, 0, :].shape, style_embeds[:, 1, :].shape, style_embeds[:, 2, :].shape)
        for idx, style_embed in enumerate([style_embeds[:, 0, :], style_embeds[:, 1, :], style_embeds[:, 2, :]]):
            if neg_content_embd is not None:
                if less_condition:
                    style_embed_s = style_embed / style_embed.norm(dim=1, keepdim=True)
                    neg_content_embd_s = neg_content_embd / (1e-6 + neg_content_embd.norm(dim=1, keepdim=True))
                    grad_zs = neg_content_embd_s.float()
                    # mask1 = ((style_embed_s * grad_zs) > torch.quantile((style_embed_s * grad_zs), 0.6, 1, True))
                    # #################### Kmeans method: determine theta automatically #####################
                    data = (style_embed_s * grad_zs).reshape(-1, 1)
                    ...
                    neg_content_embd = torch.where(mask1, style_embed, 0)
                    style_ip_tokens.append(self.style_image_proj_modules[idx](style_embed - neg_content_embd))
                else:
                    s = style_embed.norm(dim=1) / neg_content_embd.norm(dim=1)
                    # print(s.item())
                    style_ip_tokens.append(self.style_image_proj_modules[idx](style_embed - s * neg_content_embd))
            elif neg_content_name is not None:
                print("training version in exp1")
                if less_condition:
                    style_embed = style_embed - self.feature_adapter_model(style_embed)  # training version in exp1
                else:
                    text_features = text_encoder(neg_content_name.to(self.device))[0]
                    style_embed = style_embed - self.feature_adapter_model(
                        text_features.mean(1).reshape(text_features.shape[0], -1))
    ...
    def generate(
        ...
        style_ip_tokens, uncond_style_ip_tokens = self.get_image_embeds(style_image, neg_content_embd, less_condition, neg_content_name, n)
        generate_images = []
        for p in prompt:
