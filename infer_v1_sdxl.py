...
from diffusers import StableDiffusionPipeline, UniPCMultistepScheduler, StableDiffusionXLPipeline
from diffusers import AutoencoderKL, DDPMScheduler, UNet2DConditionModel
from PIL import Image

from ip_adapter import IPAdapterXL, AutoIPAdapterXL, AutoIPAdapterZS, AutoIPAdapterZSXL
...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = raw_image.resize((256, 256))  # self.transform(raw_image.convert("RGB"))
...
def collate_fn(data):
    ...
    raw_images = [example["raw_image"].convert("RGB").resize((256, 256)) for example in data]
    return {
...
def model(image, neg_content_embd, idx, args, ip_model, neg_content_name,):
    print(args.less_condition)
    if args.target_content in ["airplane", "automobile"]:
        target_content = "An {}".format(args.target_content)
    else:
        target_content = "A {}".format(args.target_content)

    neg_content_scale = 1.0

    images = ip_model.generate(pil_image=image,
                    prompt="{}, best quality, high quality".format(target_content),
                    ...
                    neg_content_name=neg_content_name,
    ...
    if args.less_condition:
        os.makedirs("outputs_SDXL/SDXL_clip_encoder_zs_{}_test/{}/ours_{}".format(neg_content_scale, args.target_content, args.theta), exist_ok=True)
        images[0].save("outputs_SDXL/SDXL_clip_encoder_zs_{}_test/{}/ours_{}/ours_{}_0.png".format(neg_content_scale, args.target_content, args.theta, idx))
    else:
        os.makedirs("outputs_SDXL/SDXL_clip_encoder_zs_{}_test/{}/base".format(neg_content_scale, args.target_content), exist_ok=True)
        images[0].save("outputs_SDXL/SDXL_clip_encoder_zs_{}_test/{}/base/base_{}_0.png".format(neg_content_scale, args.target_content, idx))
    return images[0]


def main():
    args = parse_args()
    ip_model = AutoIPAdapterZSXL(pipe, image_encoder_path, ip_ckpt, device, target_blocks=["up_blocks.0.attentions.1"])
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        pil_image = model(batch["images"], neg_content_embd, batch["image_files"][0].split(".")[0], args, ip_model, batch["texts"],)
