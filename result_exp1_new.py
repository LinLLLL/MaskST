...
class MyDataset(torch.utils.data.Dataset):
    def __init__(self, root_path="outputs", method="ours"):
        super().__init__()
    ...
    def fetch_data(self):
        objects = os.listdir(self.root_path)
        for object_name in objects:
            ...
            image_names = os.listdir(os.path.join(self.root_path, object_name, self.method))
            # for target in os.listdir(os.path.join(self.root_path, object_name)):
            #     image_names = os.listdir(os.path.join(self.root_path, object_name, target, self.method))

            for i in tqdm(range(len(image_names))):
                image_name = image_names[i]
                image_path = os.path.join(self.root_path, object_name, self.method, image_name)
                ...
                style_prompt = (image_name.split("-")[-2] + " " + image_name.split("-")[-1].split("_")[0]).replace("of",
                                                                                                                   "")
                # print(style_prompt)
                # style_prompt = tokenizer([style_prompt])

                negative_content = "An " + negative_content if negative_content in [
                    "automobile, airplane"] else "A " + negative_content
                ...
                # negative_content_prompt = text_encoder(negative_content_prompt.to(device)).text_embeds  # tokenizer([negative_content])
...
if __name__ == "__main__":
    ...
    for i, batch in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            negiative_content_features = text_encoder(negiative_content_prompts.to(device)).text_embeds  # tokenizer([negative_content])
    ...
    output_file = "results_new/exp1_metric2/style_{}_zs.xlsx".format(method)
    ...
    output_file = "results_new/exp1_metric2/text_{}_zs.xlsx".format(method)
    ...
    output_file = "results_new/exp1_metric2/neg_text_{}_zs.xlsx".format(method)
